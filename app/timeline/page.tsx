...
const EVENT_TYPES = Array.from(new Set(timelineData.events.map(e => e.type))) as EventType[];
const SAGA_ARCS = Array.from(new Set(timelineData.events.map(e => e.sagaArc).filter(Boolean))) as SagaArc[];
// PC_IDS could also be dynamic, but we stick to standard values or dynamic calculation
const PC_IDS = Array.from(new Set(timelineData.events.flatMap(e => e.pcNotes?.map(n => n.pcId) || [])));
...
export default function TimelinePage() {
  const [allEvents] = useState<TimelineEvent[]>(timelineData.events);
  ...
  const eventGroups = useMemo(() => groupEventsByArc(visibleEvents), [visibleEvents]);
  ...
  // To maintain alternating alignment across arcs
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-5xl">
                ...
                {!isCollapsed && (
                  <div className="flex flex-col">
                    {group.events.map((event) => {
                      const align = globalEventIndex % 2 === 0 ? 'left' : 'right';
                      globalEventIndex++;
      ...
      <div className="fixed bottom-8 right-8 z-50">
        ...
        {isFiltersOpen && (
          <div className="absolute bottom-20 right-0 w-80 p-6 bg-surface-container-high border border-surface-container-highest rounded-3xl shadow-2xl flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-200">
            ...
            <div>
              ...
              <select 
                className="w-full bg-surface text-on-surface text-sm rounded-xl px-4 py-3 border-none focus:ring-2 focus:ring-primary appearance-none cursor-pointer"
            ...
            <button 
              ...
              className="mt-2 w-full py-3 bg-surface hover:bg-surface-container-highest text-on-surface rounded-xl text-sm font-semibold transition-colors"
            >
              Close
            </button>
          </div>
